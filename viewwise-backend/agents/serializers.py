...
class AgentSerializer(serializers.ModelSerializer):
    ...
    download_url = serializers.SerializerMethodField(read_only=True)   # ← ICI (dans la classe)
    ...
    class Meta:
        ...
        fields = [
            ...
            'role','owner','parent_agent','download_url'
        ]
    ...
    def get_role(self, obj):
        ...
        if obj.creator == user:
            return "Éditeur"
        invitations = Invitation.objects.filter(invited_user=user, status="Accepté").order_by('-created_at')
    ...
    def get_download_url(self, obj):
        ...
        if not request:
            return None
        return request.build_absolute_uri(f'/api/agents/{obj.pk}/download-template/')
